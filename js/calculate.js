...
function calculate() {
  ...
  for (let i = 1; i <= turns; i++) {
    ...
    if (
      cardCorres[0] != cardCorres[1] &&
      cardCorres[1] != cardCorres[2] &&
      cardCorres[0] != cardCorres[2]
    ) {
      bFirstBonus = 0.5
      aFirstBonus = 1
      qFirstBonus = 0.2
    ...
    for (let j = 1; j <= 4; j++) {
      ...
      if (bl != 2) {
        ...
        if (actType == 'np') {
          ...
          if (actType == 'cr') {
            nporcrbuff = Math.max(
              (buffTotalling('cr_buff') +
                buffTotalling(cardColor + '_cr_buff')) /
                100,
              -1
            )
    ...
    totalResult.push(
      passRate(
        ...
        result[2][0],
        result[3][0],
        enemyHp - (result[0][1] + result[1][1] + result[2][1] + result[3][1])
      )
    )
