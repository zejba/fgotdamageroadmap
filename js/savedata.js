...
function exportData() {
  if (window.confirm('CSVファイルを保存しますか?')) {
    ...
    for (let i = 1; i <= turns; i++) {
      ...
      savedata.push([
        tf.getElementsByClassName('enemy-class')[0].value,
      ...
      for (let j = 1; j <= 3; j++) {
        savedata.push([
          tf.getElementsByClassName('card' + j + '-color')[0].value,
      ...
      savedata.push([
        tf.getElementsByClassName('card4-color')[0].value,
...
function getSkillData(turn, card) {
  ...
  for (let i = 0; i < buffForms.length; i++) {
    skilldata.push(
      buffForms[i]
        .getElementsByClassName('skill-name')[0]
        .value.replace(/,/g, '，'),
    ...
    if (file.type === 'text/csv') {
      ...
      reader.addEventListener('load', function () {
        ...
        for (let i = 1; i <= arr[0][17]; i++) {
          ...
          tf.getElementsByClassName('enemy-hp')[0].value = Number(
            arr[10 * i - 8][2]
          )
          tf.getElementsByClassName('dtdr')[0].value = Number(
            arr[10 * i - 8][3]
          )
          ...
          for (let j = 1; j <= 4; j++) {
            tf.getElementsByClassName('card' + j + '-color')[0].value =
              arr[10 * i + 2 * j - 8][0]
            tf.getElementsByClassName('card' + j + '-bool')[0].value = Number(
              arr[10 * i + 2 * j - 8][1]
            )
            tf.getElementsByClassName('card' + j + '-ovk')[0].value = Number(
              arr[10 * i + 2 * j - 8][2]
            )
            if (j != 4) {
              tf.getElementsByClassName('card' + j + '-cri')[0].value = Number(
                arr[10 * i + 2 * j - 8][3]
              )
            }
...
function inputSkillData(turn, card, arrc) {
  ...
  for (let i = 0; i < arrc[0]; i++) {
    ...
    buffForms[i].getElementsByClassName('amount')[0].value = Number(
      arrc[i * 5 + 3]
    )
    buffForms[i].getElementsByClassName('turn')[0].value = Number(
      arrc[i * 5 + 4]
    )
    buffForms[i].getElementsByClassName('time')[0].value = Number(
      arrc[i * 5 + 5]
    )
  }
}
...
function reflectsvData() {
  ...
  csv.open('GET', 'data/servant_data.csv?date=202508310100', false)
  csv.send(null)
  if (csv.status != 200) {
    return
  }
  ...
  let i = arr.findIndex(
    (item) => item[0] == document.getElementById('servant-list').value
  )
  if (i === -1) {
    return
  }
  ...
  for (let j = buffForms.length - 1; j >= 0; j--) {
    if (
      buffForms[j].getElementsByClassName('skill-name')[0].value ==
      'クラススキル'
    ) {
      buffForms[j].remove()
    }
  ...
  for (let j = 0; j < arr[i][16]; j++) {
    buffForms[x + j].getElementsByClassName('skill-name')[0].value =
      'クラススキル'
    buffForms[x + j].getElementsByClassName('skill-type')[0].value =
      arr[i][17 + j * 2]
    buffForms[x + j].getElementsByClassName('amount')[0].value = Number(
      arr[i][18 + j * 2]
    )
...
function reflectQData() {
  let csv = new XMLHttpRequest()
  csv.open('GET', 'data/quest_data.csv?date=202508310100', false)
  csv.send(null)
  if (csv.status != 200) {
    return
  }
  ...
  let q_id = arr.findIndex(
    (item) => item[0] == document.getElementById('quest-list').value
  )
